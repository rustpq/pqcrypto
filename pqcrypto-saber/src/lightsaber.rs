...
pub fn keypair() -> (PublicKey, SecretKey) {
    ...
    #[cfg(all(enable_aarch64_neon, feature = "neon"))]
    {
        ...
        // the moment
        if true {
            return gen_keypair!(PQCLEAN_LIGHTSABER_AARCH64_crypto_kem_keypair);
        }
    }
...
pub fn encapsulate(pk: &PublicKey) -> (SharedSecret, Ciphertext) {
    ...
    {
        if true {
            return encap!(PQCLEAN_LIGHTSABER_AARCH64_crypto_kem_enc, pk);
        }
    }
...
pub fn decapsulate(ct: &Ciphertext, sk: &SecretKey) -> SharedSecret {
    ...
    {
        if true {
            return decap!(PQCLEAN_LIGHTSABER_AARCH64_crypto_kem_dec, ct, sk);
        }
    }
