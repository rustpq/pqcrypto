...
pub struct DetachedSignature(
    ...
    [u8; ffi::PQCLEAN_DILITHIUM5_CLEAN_CRYPTO_BYTES],
    usize,
);
...
impl DetachedSignature {
    fn new() -> Self {
        DetachedSignature([0u8; ffi::PQCLEAN_DILITHIUM5_CLEAN_CRYPTO_BYTES], 0)
    }
}
...
impl primitive::DetachedSignature for DetachedSignature {
    ...
    fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let expected = ffi::PQCLEAN_DILITHIUM5_CLEAN_CRYPTO_BYTES;
        if actual > expected {
        ...
        let mut array = [0u8; ffi::PQCLEAN_DILITHIUM5_CLEAN_CRYPTO_BYTES];
...
pub fn sign(msg: &[u8], sk: &SecretKey) -> SignedMessage {
    ...
    {
        if true {
            return gen_signature!(PQCLEAN_DILITHIUM5_AARCH64_crypto_sign, msg, sk);
        }
    }
...
pub fn open(
    ...
    {
        if true {
            return open_signed!(PQCLEAN_DILITHIUM5_AARCH64_crypto_sign_open, sm, pk);
        }
    }
...
pub fn detached_sign(msg: &[u8], sk: &SecretKey) -> DetachedSignature {
    ...
    {
        if true {
            return detached_signature!(PQCLEAN_DILITHIUM5_AARCH64_crypto_sign_signature, msg, sk);
        }
    }
