...
unsafe fn keypair_avx2() -> (PublicKey, SecretKey) {
    ...
    let mut sk = SecretKey::new();
    assert_eq!(
        ffi::PQCLEAN_MCELIECE6960119_AVX2_crypto_kem_keypair(pk.0.as_mut_ptr(), sk.0.as_mut_ptr()),
        0
    );
    (pk, sk)
}
...
unsafe fn encapsulate_avx2(pk: &PublicKey) -> (SharedSecret, Ciphertext) {
    ...
    assert_eq!(
        ffi::PQCLEAN_MCELIECE6960119_AVX2_crypto_kem_enc(
            ct.0.as_mut_ptr(),
            ss.0.as_mut_ptr(),
            pk.0.as_ptr(),
        ),
...
unsafe fn decapsulate_avx2(ct: &Ciphertext, sk: &SecretKey) -> SharedSecret {
    let mut ss = SharedSecret::new();
    assert_eq!(
        ffi::PQCLEAN_MCELIECE6960119_AVX2_crypto_kem_dec(
            ss.0.as_mut_ptr(),
            ct.0.as_ptr(),
            sk.0.as_ptr(),
        ),
