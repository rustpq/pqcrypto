...
impl primitive::DetachedSignature for DetachedSignature {
    ...
    #[inline]
    fn from_bytes(bytes: &[u8]) -> Self {
        debug_assert!(bytes.len() <= ffi::PQCLEAN_SPHINCSSHAKE256128SROBUST_CLEAN_CRYPTO_BYTES);
        let mut array: [u8; ffi::PQCLEAN_SPHINCSSHAKE256128SROBUST_CLEAN_CRYPTO_BYTES] =
