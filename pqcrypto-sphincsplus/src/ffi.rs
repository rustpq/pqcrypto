...
#[cfg(enable_avx2)]
pub const PQCLEAN_SPHINCSHARAKA128SROBUST_AESNI_CRYPTO_BYTES: usize = 8080;
...
mod test_sphincsshake256192frobust_avx2 {
    ...
    unsafe fn run_test_ffi() {
        ...
        let mut pk_alt = vec![0u8; PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_CRYPTO_PUBLICKEYBYTES];
        let mut sk_alt = vec![0u8; PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_CRYPTO_SECRETKEYBYTES];
        let mut detached_sig = vec![0u8; PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_CRYPTO_BYTES];
        let mut sm = Vec::with_capacity(mlen + PQCLEAN_SPHINCSSHAKE256192FROBUST_AVX2_CRYPTO_BYTES);
        let mut smlen = 0;
        assert_eq!(
            0,
