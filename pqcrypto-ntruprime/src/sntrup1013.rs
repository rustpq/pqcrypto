...
//! # Example
//! ```
//! use pqcrypto_ntruprime::sntrup1013::*;
//! let (pk, sk) = keypair();
...
simple_struct!(
    PublicKey,
    ffi::PQCLEAN_SNTRUP1013_CLEAN_CRYPTO_PUBLICKEYBYTES
);
simple_struct!(
    SecretKey,
    ffi::PQCLEAN_SNTRUP1013_CLEAN_CRYPTO_SECRETKEYBYTES
);
simple_struct!(
    Ciphertext,
    ffi::PQCLEAN_SNTRUP1013_CLEAN_CRYPTO_CIPHERTEXTBYTES
);
simple_struct!(SharedSecret, ffi::PQCLEAN_SNTRUP1013_CLEAN_CRYPTO_BYTES);
...
pub const fn shared_secret_bytes() -> usize {
    ffi::PQCLEAN_SNTRUP1013_CLEAN_CRYPTO_BYTES
}
...
/// Generate a sntrup1013 keypair
pub fn keypair() -> (PublicKey, SecretKey) {
    #[cfg(all(enable_avx2, feature = "avx2"))]
    {
        if std::is_x86_feature_detected!("avx2") {
            return gen_keypair!(PQCLEAN_SNTRUP1013_AVX2_crypto_kem_keypair);
        }
    }
    gen_keypair!(PQCLEAN_SNTRUP1013_CLEAN_crypto_kem_keypair)
}
...
/// Encapsulate to a sntrup1013 public key
pub fn encapsulate(pk: &PublicKey) -> (SharedSecret, Ciphertext) {
    #[cfg(all(enable_avx2, feature = "avx2"))]
    {
        if std::is_x86_feature_detected!("avx2") {
            return encap!(PQCLEAN_SNTRUP1013_AVX2_crypto_kem_enc, pk);
        }
    }
    encap!(PQCLEAN_SNTRUP1013_CLEAN_crypto_kem_enc, pk)
}
...
/// Decapsulate the received sntrup1013 ciphertext
pub fn decapsulate(ct: &Ciphertext, sk: &SecretKey) -> SharedSecret {
    #[cfg(all(enable_avx2, feature = "avx2"))]
    {
        if std::is_x86_feature_detected!("avx2") {
            return decap!(PQCLEAN_SNTRUP1013_AVX2_crypto_kem_dec, ct, sk);
        }
    }
    decap!(PQCLEAN_SNTRUP1013_CLEAN_crypto_kem_dec, ct, sk)
}
