...
simple_struct!(PublicKey, ffi::PQCLEAN_FRODOKEM1344AES_CLEAN_CRYPTO_PUBLICKEYBYTES);
simple_struct!(SecretKey, ffi::PQCLEAN_FRODOKEM1344AES_CLEAN_CRYPTO_SECRETKEYBYTES);
simple_struct!(
    Ciphertext,
    ffi::PQCLEAN_FRODOKEM1344AES_CLEAN_CRYPTO_CIPHERTEXTBYTES
);
simple_struct!(SharedSecret, ffi::PQCLEAN_FRODOKEM1344AES_CLEAN_CRYPTO_BYTES);

/// Get the number of bytes for a public key
pub const fn public_key_bytes() -> usize {
    ffi::PQCLEAN_FRODOKEM1344AES_CLEAN_CRYPTO_PUBLICKEYBYTES
}

/// Get the number of bytes for a secret key
pub const fn secret_key_bytes() -> usize {
    ffi::PQCLEAN_FRODOKEM1344AES_CLEAN_CRYPTO_SECRETKEYBYTES
}
...
pub const fn shared_secret_bytes() -> usize {
    ffi::PQCLEAN_FRODOKEM1344AES_CLEAN_CRYPTO_BYTES
}
...
pub fn keypair() -> (PublicKey, SecretKey) {
    ...
    assert_eq!(
        unsafe {
            ffi::PQCLEAN_FRODOKEM1344AES_CLEAN_crypto_kem_keypair(pk.0.as_mut_ptr(), sk.0.as_mut_ptr())
        },
        0
    );
    (pk, sk)
...
pub fn encapsulate(pk: PublicKey) -> (SharedSecret, Ciphertext) {
    ...
    assert_eq!(
        unsafe {
            ffi::PQCLEAN_FRODOKEM1344AES_CLEAN_crypto_kem_enc(
                ct.0.as_mut_ptr(),
                ss.0.as_mut_ptr(),
                pk.0.as_ptr(),
            )
...
pub fn decapsulate(ct: Ciphertext, sk: SecretKey) -> SharedSecret {
    ...
    assert_eq!(
        unsafe {
            ffi::PQCLEAN_FRODOKEM1344AES_CLEAN_crypto_kem_dec(
                ss.0.as_mut_ptr(),
                ct.0.as_ptr(),
                sk.0.as_ptr(),
            )
