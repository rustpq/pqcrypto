...
//! # Example
//! ```
//! use pqcrypto_kyber::kyber51290s::*;
//! let (pk, sk) = keypair();
...
simple_struct!(
    PublicKey,
    ffi::PQCLEAN_KYBER51290S_CLEAN_CRYPTO_PUBLICKEYBYTES
);
simple_struct!(
    SecretKey,
    ffi::PQCLEAN_KYBER51290S_CLEAN_CRYPTO_SECRETKEYBYTES
);
simple_struct!(
    Ciphertext,
    ffi::PQCLEAN_KYBER51290S_CLEAN_CRYPTO_CIPHERTEXTBYTES
);
simple_struct!(SharedSecret, ffi::PQCLEAN_KYBER51290S_CLEAN_CRYPTO_BYTES);
...
pub fn keypair() -> (PublicKey, SecretKey) {
    ...
    #[cfg(all(enable_aarch64_neon, feature = "neon"))]
    {
        ...
        // the moment
        if true {
            return gen_keypair!(PQCLEAN_KYBER51290S_AARCH64_crypto_kem_keypair);
        }
    }
...
pub fn encapsulate(pk: &PublicKey) -> (SharedSecret, Ciphertext) {
    ...
    {
        if true {
            return encap!(PQCLEAN_KYBER51290S_AARCH64_crypto_kem_enc, pk);
        }
    }
...
pub fn decapsulate(ct: &Ciphertext, sk: &SecretKey) -> SharedSecret {
    ...
    {
        if true {
            return decap!(PQCLEAN_KYBER51290S_AARCH64_crypto_kem_dec, ct, sk);
        }
    }
